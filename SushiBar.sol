...
contract SushiBar is ERC20("SushiBar", "xSUSHI"){
    ...
    function canunstake() public  returns (uint256) {
        ...
        if ((block.timestamp-vestingStartTimestamp)>2 days){
            ...
            uint256 canunstakevalue = (initialTokensBalance.mul(75)).div(100);
        }else if ((block.timestamp-vestingStartTimestamp)>8 days){
            uint256 canunstakevalue =initialTokensBalance;
        }
    ...
    function leave(uint256 _share) public {
        ...
        uint256 what = _share.mul(canunstake()).div(totalShares);
